...
import os, re, shutil, threading, time, sys, configparser, os.path, exiftool
import tmdbsimple as tmdb
from colorama import Fore, Back, Style

config = configparser.ConfigParser()
config.read('<config file location>')
...
def getPERCECENTprogress(source_path, destination_path):
    ...
    if os.path.exists(destination_path):
        while os.path.getsize(source_path) != os.path.getsize(destination_path):
            ...
            sys.stdout.write(("{:d} / {:d} Mb   " .format(copiado, sizzz)) + (Style.BRIGHT + Fore.MAGENTA + "{:20s}" .format('|'*steps) + Style.RESET_ALL) + ("   {:d}% ".format(percentagem)))
            sys.stdout.flush()
            time.sleep(.01)

def sanitize_file(clean_me):
    with exiftool.ExifTool() as et:
        tags = et.get_metadata(clean_me)

    try:
        title = tags['QuickTime:Title']
    except:
        title = False
    ...
    if(title is False):
        ...
        print(Style.BRIGHT + Fore.GREEN + "Sanitizing : " + Fore.YELLOW + clean_me + Style.RESET_ALL)
        subprocess.call(['exiftool','-all=', clean_me, '-overwrite_original'])
        return
...
def CPprogress(SOURCE, DESTINATION):
    ...
    print(Style.BRIGHT + Fore.WHITE + "FROM : " + Fore.YELLOW + SOURCE + Style.RESET_ALL)
    print(Style.BRIGHT + Fore.WHITE + "TO   : " + Fore.YELLOW + dst_file + Style.RESET_ALL + "\n")
...
def deleteFile(mypath, movie):
    try:
        os.remove(os.path.join(mypath, movie))
        return
    except Exception:
        print("Failed to delete : " + os.path.join(mypath, movie))
        return
        
    

def checkfolder(movie, src_size, language):
    if(setLanguage(language) == 'delete'):
        print(Fore.RED + "Foreign Flick " + Back.WHITE + movie + Back.RESET + " : No Ones Cares, Dump It!" + Style.RESET_ALL + "\n")
        deleteFile(mypath, movie)
        return
    else:
        for folder_origin in setLanguage(language):
            ...
            if(os.path.isdir(os.path.join(folder_origin, folder_name))):
                if os.path.isfile(os.path.join(full_path, movie)):
                    print(Fore.RED + "Movie " + Back.WHITE + movie + Back.RESET + " exists! Removing!" + Style.RESET_ALL + "\n")
                    deleteFile(mypath, movie)
                else:
                    if len(os.listdir(full_path)) == 0:
                        print(Fore.RED + "Empty Folder " + Back.WHITE + movie + Back.RESET + Style.RESET_ALL + "\n")
                        CPprogress(os.path.join(mypath, movie), full_path)
                    else:
                        for mv in os.listdir(full_path):
                            x, ext = os.path.splitext(mv)
                            if(ext in {'.sub', '.sv2', '.subviewer2', '.sami', '.txt', '.nfo'}):
                                continue
                            else:
                                if (movie.endswith('.srt') or ext == '.srt'):
                                    CPprogress(os.path.join(mypath, movie), full_path)
                                else:
                                    existing_size = getSize(os.path.join(full_path, mv))
                                    if(int(src_size) < int(existing_size)):
                                        print(Fore.RED + "New Movie File " + Back.WHITE + movie + Back.RESET + " is Smaller! Replacing Existing With Smaller Version!" + Style.RESET_ALL + "\n")
                                        deleteFile(mypath, movie)
                                        CPprogress(os.path.join(mypath, movie), full_path)
                                    elif(ext not in {'.mp4'} and os.path.splitext(movie)[1] in {'.mp4'}):
                                        print(Fore.RED + "New Movie File " + Back.WHITE + movie + Back.RESET + " is an MP4 and I prefer that! Replacing : " + Style.RESET_ALL + mv + "\n")
                                        deleteFile(full_path, mv)
                                        CPprogress(os.path.join(mypath, movie), full_path)
                                    else:
                                        CPprogress(os.path.join(mypath, movie), full_path)
            else:
